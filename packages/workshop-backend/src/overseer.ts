...
class OverseerImpl implements AgentHooks {
  ...
  managedSystemOutput(): "ledger" | undefined {
    for (let gadget of this.storage.gadgets.list()) {
      if (gadget.type === "gadget" && gadget.systemOutput) return gadget.systemOutput;
    }
    return undefined;
  }
  ...
  resolveWorkpieceRoot(workpieceId?: WorkpieceId, mustExist?: boolean, forChatId?: number, forMutation?: boolean)
  ...
  async removeWorkpiece(id: WorkpieceId): Promise<void> {
    ...
    if (record.type === "gadget") this.assertGadgetMutable(id);
