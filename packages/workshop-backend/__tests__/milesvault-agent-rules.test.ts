...
describe("MilesVault agent rules", () => {
  it("keeps domain and user-data authority explicit", () => {
    expect(MILESVAULT_AGENT_RULES).toContain("through the `GRAPH` binding first");
  ...
  it("requires contract-first, scoped, evidence-backed implementation", () => {
    expect(MILESVAULT_AGENT_RULES).toContain("Before writing application files");
  ...
  it("forbids capability workarounds and unverifiable completion claims", () => {
    ...
    expect(MILESVAULT_AGENT_RULES).toContain("Never repeat an identical tool call");
