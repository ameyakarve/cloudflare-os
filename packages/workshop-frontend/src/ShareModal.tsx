...
import { absoluteAppUrl } from './appPath'
...
export default function ShareModal({ open, onClose, overseer, metadata, currentUser, authenticatedApi }: Props) {
  ...
  const workspaceUrl = absoluteAppUrl(`/workspace/${metadata.id}`)
  ...
  const handleCreateShareLink = async () => {
    ...
    try {
      ...
      const url = `${absoluteAppUrl(`/workspace/${metadata.id}`)}#share=${key}`
      setNewShareLink(url)
      setNewShareLinkCopied(false)
      setNewLinkNote('')
  ...
  const handleCopyShareLink = async (linkId: string) => {
    ...
    try {
      ...
      if (!url) {
        ...
        url = `${absoluteAppUrl(`/workspace/${metadata.id}`)}#share=${key}`
        copiedUrlsRef.current.set(linkId, url)
      }
          ...
          {sharingProhibited ? (
            <div className="flex h-full flex-col items-center justify-center px-6 py-12 text-center">
              ...
              <p className="mt-2 max-w-[320px] text-balance text-[12px] leading-[18px] tracking-[-0.1px] text-kumo-subtle">
                To share something similar, create a template from a Gadget in this workspace, then use it to create a new workspace.
              </p>
            </div>
          ) : (
          <>
