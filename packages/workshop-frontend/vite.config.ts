...
const runConfig = {
  run: {
    tasks: {
       ...
       *
       * Production mode is enforced in the config callback so the bundle command remains
       * shell-neutral and an inherited `NODE_ENV=development` cannot affect builds.
       */
      build: {
        command: ['tsc', 'tsc -p tsconfig.vite.json', 'vite build'],
